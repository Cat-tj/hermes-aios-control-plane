...
const MAIN_VIEW_PANELS = ['overview','galaxy','vps','agents','omniroute','graphify','novnc','database','discovery','runbooks','office','telemetry','safelock','settings','skills','memory','tasks','kanban','workspaces','profiles','insights','logs','plugin'];
...
async function switchPanel(name, opts = {}) {
  ...
  if (nextPanel === 'overview') await loadOverviewPanel();
  if (nextPanel === 'galaxy') await loadGalaxyPanel();
  if (nextPanel === 'tasks') await loadCrons();
  if (nextPanel === 'kanban') { try { await loadKanban(); } catch(e) { renderAiosKanbanFallback(); } }
  ...
  if (nextPanel === 'vps') await loadVpsPanel();
  if (nextPanel === 'agents') await loadAgentsPanel();
  if (nextPanel === 'office') await loadOfficePanel();
  if (nextPanel === 'database') await loadDatabasePanel();
  if (nextPanel === 'discovery') await loadDiscoveryPanel();
  if (nextPanel === 'runbooks') await loadRunbooksPanel();
  if (nextPanel === 'novnc') await loadNovncPanel();

  // Always hide compact chat box on panel switch (panels are full-width)
  if (typeof toggleCompactChatBox === 'function') {
    toggleCompactChatBox(false);
  }
...
async function loadOmniroutePanel() {
  const container = document.getElementById('omniroutePanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px">Checking Omniroute status...</div>';
  try {
    const res = await fetch('/api/omniroute/status');
    const data = await res.json();
    const isOnline = data.server_online;
    const providers = data.providers || [];
    const cc = data.claude_code || {};

    container.innerHTML = `
      <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:16px;margin-bottom:16px;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
          <h3 style="margin:0;font-size:16px;color:var(--text, #fff);display:flex;align-items:center;gap:8px">
            ⚡ Omniroute AI Gateway <span style="font-size:11px;color:var(--muted)">(Port 20128)</span>
          </h3>
          <span style="padding:4px 10px;border-radius:12px;font-size:11px;font-weight:bold;background:${isOnline ? '#10b981' : '#f59e0b'};color:#fff">
            ${isOnline ? 'ONLINE (:20128)' : 'LOCAL CONFIG ONLY'}
          </span>
        </div>
        <p style="font-size:13px;color:var(--muted, #aaa);margin:0 0 12px 0">
          Smart Multi-Provider LLM Router & Swarm Control Engine.
        </p>

        <!-- Claude Code Integration Card -->
        <div style="background:linear-gradient(135deg, rgba(99,102,241,0.15), rgba(168,85,247,0.15));border:1px solid rgba(168,85,247,0.4);border-radius:8px;padding:14px;margin-bottom:14px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-size:18px">🤖</span>
              <strong style="color:#fff;font-size:14px">Claude Code CLI Integration</strong>
              <span style="background:#8b5cf6;color:#fff;font-size:10px;padding:2px 6px;border-radius:4px">v2.1.233</span>
            </div>
            <span style="color:#10b981;font-weight:600;font-size:12px">✓ INTEGRATED & ACTIVE</span>
          </div>
          <p style="font-size:12px;color:#d1d5db;margin:0 0 8px 0">
            Claude Code CLI is fully routed through OmniRoute API Gateway (:20128) with automatic failover across AgentRouter Primary & Secondary.
          </p>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:11px">
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              <span style="color:#9ca3af">VPS Kantor (:20128):</span> <strong style="color:#38bdf8">ACTIVE (ubuntu)</strong>
            </div>
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              <span style="color:#9ca3af">VPS Altora (:20128):</span> <strong style="color:#38bdf8">ACTIVE (hermes)</strong>
            </div>
          </div>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:12px;margin-bottom:14px">
          <div style="background:var(--input-bg, #222);padding:10px;border-radius:6px">
            <span style="color:var(--muted)">Database:</span> <strong style="color:var(--text)">${data.database_connected ? 'Connected (SQLite)' : 'Not Found'}</strong>
          </div>
          <div style="background:var(--input-bg, #222);padding:10px;border-radius:6px">
            <span style="color:var(--muted)">Configured Models:</span> <strong style="color:var(--text)">${data.models_count || 0} Models</strong>
          </div>
        </div>

        <!-- Active Providers List -->
        <h4 style="margin:12px 0 8px 0;font-size:13px;color:var(--text, #fff)">Active Provider Connections (${providers.length})</h4>
        <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(220px, 1fr));gap:8px">
          ${providers.map(p => `
            <div style="background:var(--input-bg, #222);border:1px solid var(--border, #333);border-radius:6px;padding:8px 12px;display:flex;align-items:center;justify-content:space-between">
              <div>
                <div style="font-weight:600;font-size:12px;color:var(--text, #fff)">${p.name}</div>
                <div style="font-size:10px;color:var(--muted)">${p.provider} (${p.auth_type})</div>
              </div>
              <span style="font-size:10px;padding:2px 6px;border-radius:4px;background:${p.is_active ? 'rgba(16,185,129,0.2)' : 'rgba(245,158,11,0.2)'};color:${p.is_active ? '#10b981' : '#f59e0b'};font-weight:600">
      ...
      <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:16px;">
        <h4 style="margin:0 0 8px 0;font-size:14px;color:var(--text, #fff)">Quick Controls</h4>
        <p style="font-size:12px;color:var(--muted, #aaa);margin:0 0 12px 0">Manage routes and API keys from CLI or Web UI.</p>
        <button class="btn primary" onclick="window.open('http://43.134.237.147:20128', '_blank')" style="font-size:12px;padding:6px 12px">Open Omniroute Dashboard (:20128)</button>
      </div>
    `;
  } catch (err) {
    container.innerHTML = `<div style="color:#ef4444;font-size:12px">Failed to load Omniroute status: ${err.message}</div>`;
  }
}

async function loadGraphifyPanel() {
  const container = document.getElementById('graphifyPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px">Discovering Graphify knowledge graphs...</div>';
  try {
    const res = await fetch('/api/graphify/list');
    const data = await res.json();
    const projects = data.projects || [];
    if (!projects.length) {
      container.innerHTML = '<div style="color:var(--muted);font-size:12px">No Graphify outputs found. Run graphify in your project directory first!</div>';
      return;
    }
    const defaultProj = projects[0].project;
    container.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <label style="font-size:12px;color:var(--muted)">Select Knowledge Graph Project:</label>
        <select id="graphifyProjectSelect" onchange="switchGraphifyProject(this.value)" style="background:var(--input-bg, #222);color:var(--text, #fff);border:1px solid var(--border, #333);border-radius:4px;padding:4px 8px;font-size:12px">
          ${projects.map(p => `<option value="${p.project}">${p.project}</option>`).join('')}
        </select>
      </div>
      <div style="flex:1;border:1px solid var(--border, #333);border-radius:8px;overflow:hidden;background:#000">
        <iframe id="graphifyFrame" src="/api/graphify/embed?project=${defaultProj}" style="width:100%;height:100%;border:none"></iframe>
      </div>
    `;
  } catch (err) {
    container.innerHTML = `<div style="color:#ef4444;font-size:12px">Failed to load Graphify visualizer: ${err.message}</div>`;
  }
}

function switchGraphifyProject(proj) {
  const frame = document.getElementById('graphifyFrame');
  if (frame) {
    frame.src = `/api/graphify/embed?project=${proj}`;
  }
}

async function loadTelemetryPanel() {
  const container = document.getElementById('telemetryPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px">Calculating AI telemetry & token savings...</div>';
  try {
    ...
    container.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(200px, 1fr));gap:12px;margin-bottom:16px">
        <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:14px">
          <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em">Total Token Cost Savings</div>
          <div style="font-size:24px;font-weight:bold;color:#10b981;margin-top:4px">$${data.estimated_savings_usd} USD</div>
          <div style="font-size:11px;color:var(--muted);margin-top:4px">vs baseline $0.003/1k tokens</div>
        </div>
        <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:14px">
          <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em">Total Processed Tokens</div>
          <div style="font-size:24px;font-weight:bold;color:#3b82f6;margin-top:4px">${(data.total_tokens || 0).toLocaleString()}</div>
          <div style="font-size:11px;color:var(--muted);margin-top:4px">In: ${(data.total_tokens_in || 0).toLocaleString()} | Out: ${(data.total_tokens_out || 0).toLocaleString()}</div>
        </div>
        <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:14px">
          <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em">Average Response Speed</div>
          <div style="font-size:24px;font-weight:bold;color:#8b5cf6;margin-top:4px">${data.avg_latency_ms} ms</div>
          <div style="font-size:11px;color:var(--muted);margin-top:4px">Across recent ${data.total_calls} calls</div>
        </div>
      </div>

      <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:16px">
        <h4 style="margin:0 0 12px 0;font-size:14px;color:var(--text)">Model Distribution</h4>
        <div style="display:flex;flex-direction:column;gap:8px">
          ${Object.entries(data.models_distribution || {}).map(([model, count]) => `
            <div style="display:flex;align-items:center;justify-content:space-between;background:var(--input-bg, #222);padding:8px 12px;border-radius:6px;font-size:12px">
              <span style="color:var(--text);font-family:monospace">${model}</span>
              <span style="background:var(--border, #333);padding:2px 8px;border-radius:10px;color:var(--muted);font-weight:bold">${count} calls</span>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px">Failed to load telemetry stats: ${err.message}</div>`;
  }
}

async function loadSafelockPanel() {
  const container = document.getElementById('safelockPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px">Loading SafeLock credentials...</div>';
  try {
    const res = await fetch('/api/safelock/get');
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px">Error: ${data.message || 'Failed to load creds.md'}</div>`;
      return;
    }
    container.innerHTML = `
      <div style="background:var(--card-bg, #1a1a2e);border:1px solid var(--border, #333);border-radius:8px;padding:12px;display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-weight:bold;font-size:14px;color:var(--text)">🔐 SafeLock Secret Storage</div>
          <div style="font-size:11px;color:var(--muted)">File location: <code style="color:var(--text)">${data.filepath}</code></div>
        </div>
        <button class="btn primary" onclick="saveSafelockPanel()" style="font-size:12px;padding:6px 14px">Save Credentials</button>
      </div>
      <div style="flex:1;display:flex;flex-direction:column">
        <textarea id="safelockEditor" style="flex:1;width:100%;height:100%;min-height:380px;background:var(--input-bg, #0f0f1a);color:var(--text, #e0e0e0);border:1px solid var(--border, #333);border-radius:8px;padding:12px;font-family:monospace;font-size:13px;line-height:1.5;resize:none;outline:none"></textarea>
      </div>
    `;
    const editor = document.getElementById('safelockEditor');
    if (editor) editor.value = data.content || '';
  } catch (err) {
    container.innerHTML = `<div style="color:#ef4444;font-size:12px">Failed to load SafeLock credentials: ${err.message}</div>`;
  }
}

async function saveSafelockPanel() {
  const editor = document.getElementById('safelockEditor');
  if (!editor) return;
  const content = editor.value;
  try {
    const res = await fetch('/api/safelock/save', {
      method: 'POST',
    ...
    const data = await res.json();
    if (data.status === 'success') {
      alert('🔐 SafeLock creds.md successfully saved!');
    } else {
      alert('Failed to save creds.md: ' + (data.message || 'Unknown error'));
    }
  } catch (err) {
    alert('Error saving creds.md: ' + err.message);
  }
}

async function loadVpsPanel() {
  const container = document.getElementById('vpsPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Probing VPS nodes & infrastructure health...</div>';
  try {
    const res = await fetch('/api/vps/status');
    const data = await res.json();
    const sys = data.system || {};
    const cpu = sys.cpu ? sys.cpu.percent + '%' : 'N/A';
    const ram = sys.memory ? sys.memory.percent + '%' : 'N/A';
    const disk = sys.disk ? sys.disk.percent + '%' : 'N/A';

    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🖥️ VPS & Infrastructure Monitoring</h2>
            <div style="font-size:11px;color:var(--muted)">Last Checked: ${data.checked_at || 'Just now'}</div>
          </div>
          <div style="display:flex;gap:8px">
            <button onclick="loadVpsDiscoveryScan()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:5px 12px;font-size:11px;cursor:pointer;font-weight:700">🔍 Run Agentless Discovery</button>
            <button onclick="loadVpsPanel()" style="background:var(--accent-bg, rgba(8,235,241,0.1));border:1px solid var(--accent-border, rgba(8,235,241,0.3));color:#08ebf1;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:600">↻ Refresh</button>
          </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:18px">
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Online Services</div>
            <div style="font-size:18px;font-weight:700;color:#10b981;margin-top:2px">${data.summary.online_services} / ${data.summary.total_services}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">CPU Load</div>
            <div style="font-size:18px;font-weight:700;color:#08ebf1;margin-top:2px">${cpu}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Memory Used</div>
            <div style="font-size:18px;font-weight:700;color:#3889fd;margin-top:2px">${ram}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Disk Used</div>
            <div style="font-size:18px;font-weight:700;color:#a855f7;margin-top:2px">${disk}</div>
          </div>
        </div>
    ...
    (data.nodes || []).forEach(node => {
      html += `
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px 14px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
            <div>
              <span style="font-weight:700;font-size:13px;color:#fff">${node.name}</span>
              <span style="font-size:11px;color:var(--muted);margin-left:8px">(${node.ip})</span>
            </div>
            <div style="font-size:11px;color:var(--muted)">${node.role}</div>
          </div>
          <div style="display:flex;flex-direction:column;gap:6px">
      `;
      (node.services || []).forEach(s => {
        const isOnline = s.result.status === 'online';
        const statusColor = isOnline ? '#10b981' : s.result.status === 'degraded' ? '#f59e0b' : '#ef4444';
        const badgeText = isOnline ? (s.result.code ? `${s.result.code} OK` : 'ONLINE') : (s.result.error || 'OFFLINE');
        html += `
          <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(0,0,0,0.2);padding:7px 10px;border-radius:6px;font-size:12px">
            <div style="display:flex;align-items:center;gap:8px">
              ...
              <span style="color:var(--muted);font-size:11px">${s.domain}</span>
            </div>
            <div style="display:flex;align-items:center;gap:12px">
              <span style="font-size:11px;color:var(--muted)">${s.result.latency_ms}ms</span>
              <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:${statusColor}22;color:${statusColor};border:1px solid ${statusColor}44">${badgeText}</span>
            </div>
          </div>
        `;
      });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to fetch VPS status: ${err.message}</div>`;
  }
}

async function loadVpsDiscoveryScan() {
  const container = document.getElementById('vpsPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">🔍 Executing agentless discovery scan across listening ports, process table, PM2, and Git repositories...</div>';
  try {
    const res = await fetch('/api/vps/discovery');
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Discovery scan error: ${data.message}</div>`;
      return;
    }

    const summary = data.summary || {};
    const candidates = data.project_candidates || [];
    const repos = data.git_repositories || [];

    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🔍 Agentless VPS & Node Discovery Scan</h2>
            <div style="font-size:11px;color:var(--muted)">Scanned: ${data.scan_time} (${data.scan_duration_seconds}s)</div>
          </div>
          <div style="display:flex;gap:8px">
            <button onclick="loadVpsPanel()" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer">Back to Monitors</button>
            <button onclick="loadVpsDiscoveryScan()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:700">↻ Rescan Node</button>
          </div>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:18px">
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Project Candidates</div>
            <div style="font-size:18px;font-weight:700;color:#08ebf1;margin-top:2px">${summary.total_candidates || 0}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Open Ports</div>
            <div style="font-size:18px;font-weight:700;color:#10b981;margin-top:2px">${summary.discovered_ports_count || 0}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Git Repositories</div>
            <div style="font-size:18px;font-weight:700;color:#3889fd;margin-top:2px">${summary.git_repos_count || 0}</div>
          </div>
        </div>

        <h3 style="font-size:13px;font-weight:700;margin:0 0 10px;color:#fff">Correlated Project Candidates</h3>
        <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">
    `;

    candidates.forEach(c => {
      const confPercent = Math.round((c.confidence || 0) * 100);
      const confColor = confPercent >= 90 ? '#10b981' : confPercent >= 70 ? '#f59e0b' : '#ef4444';
      ...
      html += `
        <div style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:12px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-weight:700;color:#fff;font-size:13px">${c.name}</span>
              <span style="font-size:11px;color:var(--muted)">(${c.type})</span>
            </div>
            <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:${confColor}22;color:${confColor};border:1px solid ${confColor}44">${confPercent}% Confidence</span>
          </div>
          <div style="font-size:11px;color:var(--muted);margin-bottom:8px">Domain: <b style="color:#fff">${c.domain}</b> | Port: <b style="color:#08ebf1">${c.port}</b> | Path: <span style="font-family:monospace">${c.path}</span></div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
      `;
      (c.evidence || []).forEach(e => {
        html += `<span style="font-size:10px;background:rgba(8,235,241,0.08);border:1px solid rgba(8,235,241,0.2);color:#08ebf1;padding:2px 6px;border-radius:4px">✓ ${e}</span>`;
      });
      html += `</div></div>`;
    });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to run discovery scan: ${err.message}</div>`;
  }
}

async function loadAgentsPanel() {
  const container = document.getElementById('agentsPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading subagent executions & swarm telemetry...</div>';
  try {
    const res = await fetch('/api/subagents/list');
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Error: ${data.message}</div>`;
      return;
    }
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🤖 Subagents & Swarm Telemetry</h2>
            <div style="font-size:11px;color:var(--muted)">Autonomous multi-agent task execution & progress tracking</div>
          </div>
          <div style="display:flex;gap:8px">
            <button onclick="openSpawnSubagentModal()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:5px 12px;font-size:11px;cursor:pointer;font-weight:700">+ Dispatch Subagent</button>
            <button onclick="loadAgentsPanel()" style="background:var(--accent-bg, rgba(8,235,241,0.1));border:1px solid var(--accent-border, rgba(8,235,241,0.3));color:#08ebf1;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:600">↻ Refresh Telemetry</button>
          </div>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:18px">
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Total Subagents</div>
            <div style="font-size:18px;font-weight:700;color:#fff;margin-top:2px">${summary.total}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Active / Running</div>
            <div style="font-size:18px;font-weight:700;color:#08ebf1;margin-top:2px">${summary.running}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Completed</div>
            <div style="font-size:18px;font-weight:700;color:#10b981;margin-top:2px">${summary.completed}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Failed</div>
            <div style="font-size:18px;font-weight:700;color:#ef4444;margin-top:2px">${summary.failed}</div>
          </div>
        </div>

        <div style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px 14px;overflow-x:auto">
          <table style="width:100%;border-collapse:collapse;font-size:12px;text-align:left">
            <thead>
              <tr style="border-bottom:1px solid rgba(255,255,255,0.1);color:var(--muted);font-size:11px">
                <th style="padding:8px 6px">Delegation ID</th>
                <th style="padding:8px 6px">Goal / Task</th>
    ...
    if (subagents.length === 0) {
      html += `<tr><td colspan="7" style="padding:16px;text-align:center;color:var(--muted)">No subagent delegations recorded yet</td></tr>`;
    } else {
      subagents.forEach(sa => {
        const isRunning = sa.state === 'running';
        const isSuccess = sa.state === 'completed';
        const badgeBg = isSuccess ? 'rgba(16,185,129,0.15)' : isRunning ? 'rgba(8,235,241,0.15)' : 'rgba(239,68,68,0.15)';
        const badgeColor = isSuccess ? '#10b981' : isRunning ? '#08ebf1' : '#ef4444';
        const shortGoal = sa.goal.length > 80 ? sa.goal.slice(0, 80) + '...' : sa.goal;

        html += `
          <tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
            <td style="padding:10px 6px;font-family:monospace;color:#08ebf1;font-size:11px">${sa.delegation_id}</td>
            <td style="padding:10px 6px;color:#fff;max-width:280px" title="${sa.goal}">${shortGoal}</td>
            <td style="padding:10px 6px;color:var(--muted)">${sa.role} ${sa.is_batch ? '(batch)' : ''}</td>
            <td style="padding:10px 6px;color:var(--muted);font-size:11px">${sa.model}</td>
            <td style="padding:10px 6px;color:var(--muted);font-size:11px">${sa.duration_seconds}s</td>
            <td style="padding:10px 6px">
              <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:${badgeBg};color:${badgeColor}">
                ${sa.state.toUpperCase()}
              </span>
            </td>
            <td style="padding:10px 6px">
              <button onclick="loadSubagentDetailModal('${sa.delegation_id}')" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:4px;padding:3px 8px;font-size:11px;cursor:pointer">Inspect</button>
            </td>
          </tr>
        `;
      });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load subagents panel: ${err.message}</div>`;
  }
}

async function loadSubagentDetailModal(delegationId) {
  try {
    const res = await fetch(`/api/subagents/detail?id=${delegationId}`);
    const data = await res.json();
    if (data.status !== 'success' || !data.delegation) {
      alert('Subagent detail not found');
      return;
    }
...
${result.summary || result.output || JSON.stringify(result, null, 2)}
    ...
    alert('Error fetching subagent detail: ' + err.message);
  }
}

async function openSpawnSubagentModal() {
  const goal = prompt("Masukkan Goal / Tugas Subagent yang ingin dijalankan:");
  ...
  try {
    const res = await fetch('/api/subagents/spawn', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ goal: goal.trim(), role: 'leaf' })
    });
    const data = await res.json();
    if (data.status === 'success') {
      alert(`🚀 Subagent Berhasil Di-dispatch!\nID: ${data.delegation_id}\nPID: ${data.pid}`);
      loadAgentsPanel();
    } else {
      alert('Gagal dispatch subagent: ' + (data.message || 'Unknown error'));
    }
  } catch (err) {
    alert('Error dispatching subagent: ' + err.message);
  }
}

function getActiveAiosProject() {
  return localStorage.getItem('aios-active-project') || 'all';
}

async function loadOfficePanel() {
  const container = document.getElementById('officePanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading virtual agent office workspace...</div>';
  try {
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🏢 Virtual Agent Office & Multi-Agent Swarm</h2>
            <div style="font-size:11px;color:var(--muted)">Visual workspace: Project Lead Bots & dynamic child subagent execution trees</div>
          </div>
          <button onclick="loadOfficePanel()" style="background:var(--accent-bg, rgba(8,235,241,0.1));border:1px solid var(--accent-border, rgba(8,235,241,0.3));color:#08ebf1;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:600">↻ Refresh Office</button>
        </div>

        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-bottom:18px">
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Project Desks</div>
            <div style="font-size:18px;font-weight:700;color:#fff;margin-top:2px">${data.total_desks}</div>
          </div>
          <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px">
            <div style="font-size:11px;color:var(--muted)">Active Agent Swarms</div>
            <div style="font-size:18px;font-weight:700;color:#08ebf1;margin-top:2px">${data.active_swarms}</div>
          </div>
        </div>
    ...
    desks.forEach(d => {
      const isBusy = d.stats.active > 0;
      const glowColor = d.color || '#08ebf1';
      const statusText = isBusy ? `⚡ EXECUTING (${d.stats.active} active subagent${d.stats.active > 1 ? 's' : ''})` : '🟢 IDLE / READY';
      const statusBg = isBusy ? `${glowColor}22` : 'rgba(255,255,255,0.05)';

      html += `
        <div style="background:rgba(255,255,255,0.02);border:1px solid ${isBusy ? glowColor : 'rgba(255,255,255,0.08)'};border-radius:12px;padding:14px;box-shadow:${isBusy ? '0 0 12px ' + glowColor + '33' : 'none'}">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.06);padding-bottom:10px">
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <div>
                <div style="font-weight:700;font-size:14px;color:#fff">${d.name}</div>
                <div style="font-size:11px;color:var(--muted)">${d.description}</div>
              </div>
            </div>
            <div style="display:flex;align-items:center;gap:10px">
              <span style="font-size:10px;font-weight:700;padding:3px 8px;border-radius:6px;background:${statusBg};color:${glowColor};border:1px solid ${glowColor}44">${statusText}</span>
              <button onclick="dispatchToProjectModal('${d.id}')" style="background:${glowColor};border:none;color:#000;border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;font-weight:700">+ Dispatch Subagent</button>
            </div>
          </div>

          <!-- Project Lead Bot Card -->
          <div style="display:flex;align-items:center;gap:12px;background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px 12px;margin-bottom:10px">
            <div style="width:36px;height:36px;border-radius:50%;background:${glowColor}22;border:1px solid ${glowColor};display:flex;align-items:center;justify-content:center;font-size:18px">🤖</div>
            <div style="flex:1">
              <div style="font-size:12px;font-weight:700;color:#fff">${d.lead_bot.name}</div>
              <div style="font-size:11px;color:var(--muted)">Status: ${d.lead_bot.status} | Total Spawned: ${d.stats.total} subagents</div>
            </div>
          </div>

          <!-- Subagent Workers Tree -->
          <div style="margin-left:20px;padding-left:14px;border-left:2px dashed ${glowColor}55">
            <div style="font-size:11px;font-weight:700;color:var(--muted);margin-bottom:8px">ACTIVE WORKER SUBAGENTS (${d.subagent_workers.length})</div>
      `;

      if (d.subagent_workers.length === 0) {
        html += `<div style="font-size:11px;color:var(--muted);padding:6px 0">No subagents actively running on this desk</div>`;
      } else {
        d.subagent_workers.forEach(w => {
          const wIsRunning = w.state === 'running';
          const wColor = wIsRunning ? '#08ebf1' : w.state === 'completed' ? '#10b981' : '#ef4444';
          const shortGoal = w.goal.length > 70 ? w.goal.slice(0, 70) + '...' : w.goal;

          html += `
            <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.06);border-radius:6px;padding:8px 10px;margin-bottom:6px;font-size:11px">
              <div style="display:flex;align-items:center;gap:8px">
                <span style="font-size:14px">🤖</span>
                <div>
                  <span style="font-family:monospace;color:${wColor};font-weight:700">${w.delegation_id}</span>
                  <span style="color:#fff;margin-left:6px">${shortGoal}</span>
                </div>
              </div>
              <div style="display:flex;align-items:center;gap:10px">
                <span style="color:var(--muted)">${w.duration_seconds}s</span>
                <span style="font-weight:700;color:${wColor}">${w.state.toUpperCase()}</span>
                <button onclick="loadSubagentDetailModal('${w.delegation_id}')" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:4px;padding:2px 6px;font-size:10px;cursor:pointer">Inspect</button>
              </div>
            </div>
          `;
        });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load Office panel: ${err.message}</div>`;
  }
}

async function dispatchToProjectModal(projectId) {
  const goal = prompt(`Masukkan Goal / Tugas Subagent khusus untuk proyek [${projectId.toUpperCase()}]:`);
  if (!goal || !goal.trim()) return;
  ...
  try {
    const res = await fetch('/api/office/dispatch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ goal: goal.trim(), project_id: projectId, role: 'leaf' })
    });
    const data = await res.json();
    if (data.status === 'success') {
      alert(`🚀 Subagent Berhasil Di-dispatch ke Desk [${projectId.toUpperCase()}]!\nID: ${data.delegation_id}\nPID: ${data.pid}`);
      loadOfficePanel();
    } else {
      alert('Gagal dispatch subagent: ' + (data.message || 'Unknown error'));
    }
...
function onAiosProjectChange(val) {
  localStorage.setItem('aios-active-project', val);
  console.log('[AIOS] Active Project changed to:', val);
  if (typeof _currentPanel !== 'undefined') {
    if (_currentPanel === 'vps' && typeof loadVpsPanel === 'function') loadVpsPanel();
    if (_currentPanel === 'agents' && typeof loadAgentsPanel === 'function') loadAgentsPanel();
    if (_currentPanel === 'office' && typeof loadOfficePanel === 'function') loadOfficePanel();
    if (_currentPanel === 'kanban' && typeof loadKanban === 'function') loadKanban(true);
  }
}

async function loadDatabasePanel() {
  const container = document.getElementById('databasePanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading database connections & schema explorer...</div>';
  
  try {
    const activeProject = getActiveAiosProject();
    const res = await fetch(`/api/db/list?project=${activeProject}`);
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Error: ${data.message}</div>`;
      return;
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🗄️ Database Explorer & Schema Management</h2>
            <div style="font-size:11px;color:var(--muted)">Read-only SQL queries, schema inspection, and project database connections</div>
          </div>
          <button onclick="loadDatabasePanel()" style="background:var(--accent-bg, rgba(8,235,241,0.1));border:1px solid var(--accent-border, rgba(8,235,241,0.3));color:#08ebf1;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:600">↻ Refresh DBs</button>
        </div>
    ...
    dbs.forEach(db => {
      ...
      html += `
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:12px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
            <span style="font-weight:700;font-size:13px;color:#fff">${db.name}</span>
            <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:${statusColor}22;color:${statusColor}">${db.status.toUpperCase()}</span>
          </div>
          <div style="font-size:11px;color:var(--muted);font-family:monospace;margin-bottom:8px;word-break:break-all">${db.path}</div>
          <div style="display:flex;align-items:center;justify-content:space-between;font-size:11px">
            <span style="color:var(--muted)">Size: ${db.size_mb} MB</span>
            <button onclick="loadDbSchemaExplorer('${db.id}')" style="background:rgba(8,235,241,0.1);border:1px solid rgba(8,235,241,0.3);color:#08ebf1;border-radius:4px;padding:3px 8px;font-size:11px;cursor:pointer;font-weight:600">Inspect Schema</button>
          </div>
        </div>
      `;
    });
    ...
    html += `
        ...
        <div id="dbSchemaExplorerView">
          <div style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:14px">
            <h3 style="font-size:13px;font-weight:700;margin:0 0 10px;color:#fff">⚡ SQL Query Runner (Read-Only)</h3>
            <div style="display:flex;gap:8px;margin-bottom:10px">
              <select id="dbSelectTarget" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:6px;padding:6px 10px;font-size:12px">
                ${dbs.map(d => `<option value="${d.id}">${d.name}</option>`).join('')}
              </select>
              <input id="sqlQueryInput" type="text" placeholder="SELECT * FROM async_delegations ORDER BY dispatched_at DESC LIMIT 10;" style="flex:1;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:6px;padding:6px 12px;font-size:12px;font-family:monospace">
              <button onclick="runDbQuery()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer">Run Query</button>
            </div>
            <div id="sqlQueryResult" style="overflow-x:auto"></div>
          </div>
        </div>
      </div>
    `;
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load database explorer: ${err.message}</div>`;
  }
}

async function loadDbSchemaExplorer(dbId) {
  const resultDiv = document.getElementById('sqlQueryResult');
  if (!resultDiv) return;
  resultDiv.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:10px">Loading table schema...</div>';
  try {
    const res = await fetch(`/api/db/schema?id=${dbId}`);
    const data = await res.json();
    if (data.status !== 'success') {
      resultDiv.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:10px">Error: ${data.message}</div>`;
      return;
    }

    let html = `
      <div style="margin-top:10px">
        <h4 style="font-size:12px;font-weight:700;color:#08ebf1;margin:0 0 8px">Tables in [${data.db_name}]:</h4>
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">
    `;
    (data.tables || []).forEach(t => {
      html += `
        <button onclick="document.getElementById('sqlQueryInput').value='SELECT * FROM \\'${t.name}\\' LIMIT 20;'; runDbQuery();" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.12);color:#fff;border-radius:4px;padding:4px 8px;font-size:11px;cursor:pointer">
    ...
    resultDiv.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:10px">Failed: ${err.message}</div>`;
  }
}

async function runDbQuery() {
  const dbId = document.getElementById('dbSelectTarget').value;
  const query = document.getElementById('sqlQueryInput').value;
  const resultDiv = document.getElementById('sqlQueryResult');
  if (!query || !query.trim()) return;
  
  resultDiv.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:10px">Executing query...</div>';
  try {
    const res = await fetch('/api/db/query', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: dbId, query: query.trim() })
    });
    const data = await res.json();
    if (data.status !== 'success') {
      resultDiv.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:10px">Query Error: ${data.message}</div>`;
      return;
    }
    ...
    if (rows.length === 0) {
      resultDiv.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:10px">Query executed successfully. 0 rows returned.</div>';
      return;
    }

    let html = `
      <div style="font-size:11px;color:#10b981;margin-bottom:6px;font-weight:600">✓ Returned ${data.row_count} rows</div>
      <table style="width:100%;border-collapse:collapse;font-size:11px;font-family:monospace">
        <thead>
          <tr style="background:rgba(255,255,255,0.06);color:#08ebf1">
            ${cols.map(c => `<th style="padding:6px 8px;border:1px solid rgba(255,255,255,0.1);text-align:left">${c}</th>`).join('')}
          </tr>
        </thead>
        <tbody>
    `;
    ...
    rows.forEach(r => {
      ...
      cols.forEach(c => {
        ...
        html += `<td style="padding:6px 8px;border:1px solid rgba(255,255,255,0.05);color:#fff;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${val}">${displayVal}</td>`;
      });
      html += '</tr>';
    });
    ...
    resultDiv.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:10px">Failed to execute query: ${err.message}</div>`;
  }
}

document.addEventListener('DOMContentLoaded', () => {
  const sel = document.getElementById('aiosProjectSelector');
  if (sel) {
    sel.value = getActiveAiosProject();
  }
});

async function loadDiscoveryPanel() {
  const container = document.getElementById('discoveryPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Running agentless VPS discovery scan & evidence correlation...</div>';
  try {
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">🔍 Agentless VPS Discovery & Candidate Correlation</h2>
            <div style="font-size:11px;color:var(--muted)">Evidence-first auto-discovery for VPS nodes & local runtime containers</div>
          </div>
          <button onclick="loadDiscoveryPanel()" style="background:var(--accent-bg, rgba(8,235,241,0.1));border:1px solid var(--accent-border, rgba(8,235,241,0.3));color:#08ebf1;border-radius:6px;padding:5px 10px;font-size:11px;cursor:pointer;font-weight:600">↻ Rescan VPS Nodes</button>
        </div>
        <div style="display:flex;flex-direction:column;gap:14px">
    `;
      ...
      html += `
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:14px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;border-bottom:1px solid rgba(255,255,255,0.06);padding-bottom:8px">
            <div>
              <span style="font-weight:700;font-size:14px;color:#fff">${node.name}</span>
              <span style="font-size:11px;color:#08ebf1;margin-left:8px">(${node.ip})</span>
            </div>
            <span style="font-size:11px;color:var(--muted)">Last scan: ${node.last_scanned_at}</span>
          </div>
          <div style="display:flex;flex-direction:column;gap:8px">
      `;

      (node.discovered_services || []).forEach(svc => {
        const confPct = Math.round((svc.confidence || 0) * 100);
        html += `
          <div style="background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.06);border-radius:8px;padding:10px">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
              <div style="display:flex;align-items:center;gap:8px">
                <span style="font-weight:700;color:#fff;font-size:13px">${svc.project}</span>
                <span style="font-size:10px;padding:2px 6px;border-radius:4px;background:rgba(8,235,241,0.15);color:#08ebf1;font-weight:600">${svc.type}</span>
              </div>
              <span style="font-size:11px;color:#10b981;font-weight:700">${confPct}% Confidence</span>
            </div>
            <div style="font-size:11px;color:var(--muted);margin-bottom:6px">Path: <code style="color:#a855f7">${svc.path}</code> | Port: <code style="color:#08ebf1">${svc.port}</code> ${svc.domain ? '| Domain: ' + svc.domain : ''}</div>
            <div style="font-size:11px;color:var(--muted);background:rgba(0,0,0,0.2);padding:6px;border-radius:4px">
              <strong>Evidence:</strong> ${(svc.evidence || []).join(' • ')}
            </div>
          </div>
        `;
      });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load discovery panel: ${err.message}</div>`;
  }
}

async function loadRunbooksPanel() {
  const container = document.getElementById('runbooksPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading safe runbooks & operational action catalog...</div>';
  try {
    const res = await fetch('/api/runbooks/list');
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Error: ${data.message}</div>`;
      return;
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;margin:0 0 2px;color:#fff">📋 Safe Operations & Typed Runbooks</h2>
            <div style="font-size:11px;color:var(--muted)">Audited, policy-gated operational execution workflows</div>
          </div>
        </div>
    ...
    runbooks.forEach(rb => {
      const isL3 = rb.risk_level.includes('L3');
      const badgeBg = isL3 ? 'rgba(245,158,11,0.15)' : 'rgba(16,185,129,0.15)';
      const badgeColor = isL3 ? '#f59e0b' : '#10b981';

      html += `
        <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px;display:flex;flex-direction:column;justify-content:space-between">
          <div>
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
              <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:${badgeBg};color:${badgeColor}">${rb.risk_level}</span>
              <span style="font-size:11px;color:var(--muted)">${rb.category}</span>
            </div>
            <h3 style="font-size:13px;font-weight:700;color:#fff;margin:0 0 4px">${rb.name}</h3>
            <p style="font-size:11px;color:var(--muted);margin:0 0 10px">${rb.description}</p>
          </div>
          <button onclick="executeRunbookAction('${rb.id}')" style="width:100%;background:rgba(8,235,241,0.1);border:1px solid rgba(8,235,241,0.3);color:#08ebf1;border-radius:6px;padding:6px;font-size:11px;font-weight:600;cursor:pointer">Execute Runbook</button>
        </div>
      `;
    });
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load runbooks panel: ${err.message}</div>`;
  }
}

async function executeRunbookAction(rbId) {
  try {
    const res = await fetch('/api/runbooks/execute', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: rbId, params: {} })
    });
    const data = await res.json();
    alert(`Runbook Result (${data.status}):\nAudit ID: ${data.audit_id || 'N/A'}\n` + JSON.stringify(data.result || data.approval_gate || data.message, null, 2));
  } catch (err) {
    alert('Failed to execute runbook: ' + err.message);
  }
}

let _compactChatClosedByUser = false;

function toggleCompactChatBox(show) {
  const box = document.getElementById('compactChatBox');
...
function toggleChatDrawer(forceOpen) {
  ...
  if (!drawer) return;
  _chatDrawerOpen = typeof forceOpen === 'boolean' ? forceOpen : !_chatDrawerOpen;
  drawer.style.display = _chatDrawerOpen ? 'flex' : 'none';
  const layout = document.querySelector('.layout');
  if (layout) layout.classList.toggle('chat-drawer-open', _chatDrawerOpen);
}

function sendDrawerChatMessage() {
  const inp = document.getElementById('drawerChatInput');
  if (!inp || !inp.value.trim()) return;
  const txt = inp.value.trim();
  inp.value = '';

  const msgContainer = document.getElementById('drawerChatMessages');
  if (msgContainer) {
    const userMsg = document.createElement('div');
    userMsg.style.cssText = 'background:rgba(8,235,241,0.15);border:1px solid rgba(8,235,241,0.3);color:#fff;padding:8px 12px;border-radius:8px;margin-bottom:8px';
    userMsg.textContent = txt;
    msgContainer.appendChild(userMsg);

    const botMsg = document.createElement('div');
    botMsg.style.cssText = 'background:rgba(255,255,255,0.05);border:1px solid var(--border,#333);color:#d1d5db;padding:8px 12px;border-radius:8px;margin-bottom:8px';
    botMsg.textContent = 'Raphael: Processing request via OmniRoute...';
    msgContainer.appendChild(botMsg);
    msgContainer.scrollTop = msgContainer.scrollHeight;
  }
}

// ── URL & Deep Link Router Synchronization ──
function _syncUrlWithPanel(panelName) {
  if (!panelName) return;
  try {
    const url = new URL(window.location.href);
    if (url.searchParams.get('panel') !== panelName) {
...
window.addEventListener('popstate', (e) => {
  ...
  const panel = params.get('panel') || (e.state && e.state.panel) || 'overview';
  if (typeof switchPanel === 'function') switchPanel(panel, { bypassSettingsGuard: true });
});
...
function sendCompactChatMessage() {
  const inp = document.getElementById('compactChatInput');
  if (!inp || !inp.value.trim()) return;
  const txt = inp.value.trim();
  inp.value = '';

  const msgContainer = document.getElementById('compactChatMessages');
  if (msgContainer) {
    const userMsgEl = document.createElement('div');
    userMsgEl.style.cssText = 'align-self:flex-end;background:rgba(8,235,241,0.15);border:1px solid rgba(8,235,241,0.3);color:#fff;padding:6px 10px;border-radius:8px;max-width:85%;word-break:break-word';
    userMsgEl.textContent = txt;
...
async function renderAiosKanbanFallback(container, projectFilter) {
  if (!container) container = document.getElementById('kanbanBoard') || document.getElementById('panelKanban');
  if (!container) return;

  const proj = projectFilter || getActiveAiosProject();
  try {
    const res = await fetch(`/api/kanban/list?project=${proj}`);
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Kanban Error: ${data.message}</div>`;
      return;
    }
    ...
    const cols = [
      ...
      { id: 'in_progress', title: '⚡ In Progress', color: '#08ebf1' },
      { id: 'review', title: '🔍 Review & Verification', color: '#f59e0b' },
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
          <div>
            <h2 style="font-size:16px;font-weight:700;color:#fff;margin:0 0 2px">🎯 Project Tracking & Kanban (${proj.toUpperCase()})</h2>
            <div style="font-size:11px;color:var(--muted)">Evidence-first task tracking per project and subagent execution</div>
          </div>
          <button onclick="promptCreateKanbanTask()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer">+ New Task</button>
        </div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px">
    `;

    cols.forEach(col => {
      const colTasks = tasks.filter(t => (t.column_id || 'backlog') === col.id);
      html += `
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px;display:flex;flex-direction:column;gap:8px">
          <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,0.06);padding-bottom:8px">
            <span style="font-weight:700;font-size:13px;color:${col.color}">${col.title}</span>
            <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:10px;background:rgba(255,255,255,0.05);color:#fff">${colTasks.length}</span>
          </div>
          <div style="display:flex;flex-direction:column;gap:8px;min-height:120px">
      `;

      if (colTasks.length === 0) {
        html += `<div style="font-size:11px;color:var(--muted);text-align:center;padding:20px 0">No tasks</div>`;
      } else {
        colTasks.forEach(t => {
          const prioBg = t.priority === 'high' ? 'rgba(239,68,68,0.15)' : 'rgba(255,255,255,0.05)';
          const prioColor = t.priority === 'high' ? '#ef4444' : 'var(--muted)';
          html += `
            <div style="background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px">
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
                <span style="font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;background:rgba(8,235,241,0.1);color:#08ebf1">${(t.project_id || 'aios').toUpperCase()}</span>
                <span style="font-size:10px;padding:1px 4px;border-radius:4px;background:${prioBg};color:${prioColor}">${t.priority.toUpperCase()}</span>
              </div>
              <div style="font-size:12px;font-weight:600;color:#fff;margin-bottom:4px">${t.title}</div>
              <div style="font-size:11px;color:var(--muted);margin-bottom:8px">${t.description}</div>
              <div style="display:flex;align-items:center;justify-content:space-between;font-size:10px">
                <span style="color:var(--muted)">👤 ${t.assignee || 'unassigned'}</span>
                <div style="display:flex;gap:4px">
                  ${col.id !== 'backlog' ? `<button onclick="moveKanbanTask('${t.id}', '${col.id}', 'prev')" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:3px;padding:1px 5px;cursor:pointer">←</button>` : ''}
                  ${col.id !== 'completed' ? `<button onclick="moveKanbanTask('${t.id}', '${col.id}', 'next')" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#fff;border-radius:3px;padding:1px 5px;cursor:pointer">→</button>` : ''}
                </div>
              </div>
            </div>
          `;
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load Kanban: ${err.message}</div>`;
  }
}

function promptCreateKanbanTask() {
  const title = prompt("Task Title / Name:");
  if (!title || !title.trim()) return;
  const description = prompt("Task Description:") || "";
  const proj = getActiveAiosProject();
...
function moveKanbanTask(taskId, currentCol, direction) {
  ...
  fetch('/api/kanban/save', {
    ...
    body: JSON.stringify({
      id: taskId,
      project_id: getActiveAiosProject(),
      column_id: nextCol
    })
  }).then(r => r.json()).then(res => {
    if (res.status === 'success') {
      ...
      alert("Failed to move task: " + res.message);
    }
  });
}

async function loadOverviewPanel() {
  const container = document.getElementById('overviewPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading Raphael Command Center status & alerts...</div>';

  try {
    const [ovRes, errRes, hierRes, agRes] = await Promise.all([
      fetch('/api/overview/status').then(r => r.json()),
      fetch('/api/errors/list').then(r => r.json()),
      fetch('/api/domain/hierarchy').then(r => r.json()),
      fetch('/api/heartbeat/agents').then(r => r.json())
    ]);

    const s = ovRes.summary || {};
    const services = ovRes.services || [];
    const errCounts = (errRes.counts) || { CRITICAL: 0, ERROR: 0, WARNING: 0, INFO: 0, TOTAL: 0 };
    const errorsList = errRes.errors || [];
    const pillars = hierRes.pillars || [];
    const agentSummary = (agRes.summary) || {};
    const agentsList = agRes.agents || [];

    let html = `
      <div style="padding:16px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">
          <div>
            <h2 style="font-size:18px;font-weight:700;color:#fff;margin:0 0 2px">👑 Raphael Control Center — Company Command Dashboard</h2>
            <div style="font-size:11px;color:var(--muted)">Unified company operating control plane for projects, missions, agents & infrastructure</div>
          </div>
          <button onclick="loadOverviewPanel()" style="background:rgba(8,235,241,0.1);border:1px solid rgba(8,235,241,0.3);color:#08ebf1;border-radius:6px;padding:6px 12px;font-size:11px;cursor:pointer;font-weight:600">↻ Refresh Command Dashboard</button>
        </div>

        <!-- GLOBAL WARNING & ATTENTION CENTER BAR -->
        <div style="background:rgba(22,27,34,0.8);border:1px solid rgba(239,68,68,0.3);border-radius:12px;padding:14px;margin-bottom:20px;box-shadow:0 4px 20px rgba(0,0,0,0.4)">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
            <div style="font-size:13px;font-weight:700;color:#fff;display:flex;align-items:center;gap:6px">
              <span>⚠️ NEEDS ATTENTION</span>
              <span style="font-size:11px;color:var(--muted);font-weight:400">(${errCounts.TOTAL} active issues)</span>
            </div>
            <div style="display:flex;gap:8px;font-size:11px;font-weight:700">
              <span style="background:rgba(239,68,68,0.2);color:#ef4444;padding:3px 8px;border-radius:6px;border:1px solid rgba(239,68,68,0.3)">🔴 ${errCounts.CRITICAL} Critical</span>
              <span style="background:rgba(249,115,22,0.2);color:#f97316;padding:3px 8px;border-radius:6px;border:1px solid rgba(249,115,22,0.3)">🟠 ${errCounts.ERROR} Errors</span>
              <span style="background:rgba(245,158,11,0.2);color:#f59e0b;padding:3px 8px;border-radius:6px;border:1px solid rgba(245,158,11,0.3)">🟡 ${errCounts.WARNING} Warnings</span>
            </div>
          </div>

          <div style="display:flex;flex-direction:column;gap:8px">
            ${errorsList.length === 0 ? '<div style="font-size:11px;color:#10b981;padding:4px 0">🟢 All systems operational. Zero active critical issues or warnings.</div>' : ''}
            ${errorsList.slice(0, 4).map(err => {
              const bg = err.severity === 'CRITICAL' ? 'rgba(239,68,68,0.1)' : (err.severity === 'ERROR' ? 'rgba(249,115,22,0.1)' : 'rgba(245,158,11,0.1)');
              const border = err.severity === 'CRITICAL' ? '#ef4444' : (err.severity === 'ERROR' ? '#f97316' : '#f59e0b');
              return `
                <div style="background:${bg};border-left:4px solid ${border};border-radius:6px;padding:8px 12px;display:flex;align-items:center;justify-content:space-between">
                  <div>
                    <div style="font-size:12px;font-weight:700;color:#fff">${err.id}: ${err.title} <span style="font-size:10px;color:var(--muted);font-weight:400">(${err.occurrences}x)</span></div>
                    <div style="font-size:11px;color:var(--muted);margin-top:2px">Source: <strong>${err.source}</strong> | Resource: <code>${err.affected_resource || 'N/A'}</code></div>
                    <div style="font-size:10px;color:#94a3b8;margin-top:2px">Evidence: ${err.evidence || 'N/A'}</div>
                  </div>
                  <div style="display:flex;gap:6px">
                    <button onclick="updateErrorStatusAction('${err.id}', 'INVESTIGATING')" style="background:rgba(8,235,241,0.15);border:1px solid rgba(8,235,241,0.3);color:#08ebf1;border-radius:4px;padding:4px 8px;font-size:10px;font-weight:700;cursor:pointer">Investigate</button>
                    <button onclick="updateErrorStatusAction('${err.id}', 'RESOLVED')" style="background:rgba(16,185,129,0.15);border:1px solid rgba(16,185,129,0.3);color:#10b981;border-radius:4px;padding:4px 8px;font-size:10px;font-weight:700;cursor:pointer">Resolve</button>
                  </div>
                </div>
              `;
            }).join('')}
        ...
        <!-- COMPANY PILLARS & PROJECTS HIERARCHY -->
        <h3 style="font-size:14px;font-weight:700;color:#fff;margin:0 0 10px">🏛️ Company Pillars & Active Projects</h3>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px;margin-bottom:20px">
          ${pillars.map(p => `
            <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:12px">
              <div style="font-size:13px;font-weight:700;color:#08ebf1;margin-bottom:4px">Pillar: ${p.name}</div>
              <div style="font-size:11px;color:var(--muted);margin-bottom:10px">${p.description} (Lead: ${p.lead_person})</div>

              <div style="display:flex;flex-direction:column;gap:8px">
                ${(p.projects || []).map(proj => `
                  <div style="background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.06);border-radius:6px;padding:8px">
                    <div style="display:flex;align-items:center;justify-content:space-between">
                      <span style="font-size:12px;font-weight:700;color:#fff">${proj.name}</span>
                      <span style="font-size:10px;padding:2px 6px;border-radius:4px;background:rgba(16,185,129,0.15);color:#10b981;font-weight:700">${proj.status.toUpperCase()}</span>
                    </div>
                    <div style="font-size:11px;color:var(--muted);margin:4px 0">${proj.description}</div>
                    ${proj.id === 'galaxy' ? `
                      <button onclick="switchPanel('galaxy')" style="background:linear-gradient(135deg, #08ebf1, #3b82f6);color:#000;border:none;border-radius:6px;padding:8px 12px;font-size:11px;font-weight:800;cursor:pointer;margin:6px 0;width:100%;box-shadow:0 2px 10px rgba(8,235,241,0.3)">🌌 Launch Project Galaxy Visualizer</button>
                    ` : ''}
                    ${(proj.missions || []).map(m => `
                      <div style="font-size:11px;color:#cbd5e1;margin-top:4px;display:flex;align-items:center;justify-content:space-between">
                        <span>🎯 Mission: <strong>${m.title}</strong></span>
                        <span style="font-size:10px;color:#08ebf1;font-weight:700">${m.milestone_summary}</span>
                      </div>
                    `).join('')}
                  </div>
                `).join('')}
        ...
        <!-- SPECIALIST AGENTS & LIVENESS TELEMETRY -->
        <h3 style="font-size:14px;font-weight:700;color:#fff;margin:0 0 10px">🤖 Agent Workforce Liveness (${agentSummary.TOTAL} total)</h3>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin-bottom:20px">
          ${agentsList.map(ag => {
            const st = ag.liveness_status || 'IDLE';
            const icon = st === 'WORKING' ? '🟢' : (st === 'IDLE' ? '🔵' : (st === 'WAITING' ? '🟡' : (st === 'BLOCKED' ? '🟠' : (st === 'OFFLINE' ? '⚫' : '🔴'))));
            const color = st === 'WORKING' ? '#10b981' : (st === 'IDLE' ? '#3b82f6' : (st === 'WAITING' ? '#f59e0b' : '#ef4444'));
            return `
              <div style="background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:10px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
                  <span style="font-size:12px;font-weight:700;color:#fff">${ag.name}</span>
                  <span style="font-size:10px;font-weight:700;color:${color}">${icon} ${st}</span>
                </div>
                <div style="font-size:10px;color:var(--muted);margin-bottom:4px">Scope: <strong>${ag.scope}</strong> ${ag.project_id ? `(${ag.project_id})` : ''}</div>
                <div style="font-size:10px;color:#94a3b8">Role: ${ag.role}</div>
              </div>
            `;
          }).join('')}
        </div>
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load Raphael Command Center: ${err.message}</div>`;
  }
}

async function updateErrorStatusAction(errId, newStatus) {
  try {
    const res = await fetch('/api/errors/status', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: errId, status: newStatus })
    });
    const data = await res.json();
    if (data.status === 'success') {
      loadOverviewPanel();
    } else {
      alert("Failed to update error status: " + data.message);
    }
  } catch (e) {
...
async function loadGalaxyPanel() {
  const container = document.getElementById('mainGalaxy');
  if (!container) return;
...
async function loadNovncPanel() {
  const container = document.getElementById('novncPanelContent');
  if (!container) return;
  container.innerHTML = '<div style="color:var(--muted);font-size:12px;padding:12px">Loading noVNC remote desktop servers & web gateway...</div>';

  try {
    const res = await fetch('/api/novnc/list');
    const data = await res.json();
    if (data.status !== 'success') {
      container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Error: ${data.message}</div>`;
      return;
    ...
    let html = `
      <div style="padding:14px;font-family:system-ui,-apple-system,sans-serif;color:var(--text, #e8e8f0);display:flex;flex-direction:column;height:100%">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
          <div>
            <h2 style="font-size:16px;font-weight:700;color:#fff;margin:0 0 2px">🖥️ noVNC Remote Desktop Gateway</h2>
            <div style="font-size:11px;color:var(--muted)">HTML5 VNC & WebSockets remote desktop access for Office Server, VPS, & local machines</div>
          </div>
          <button onclick="promptAddNovncServer()" style="background:#08ebf1;border:none;color:#000;border-radius:6px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer">+ Add VNC Server</button>
        </div>

        <div style="display:flex;gap:10px;margin-bottom:12px">
          <select id="novncSelectServer" style="background:rgba(255,255,255,0.06);border:1px solid rgba(8,235,241,0.3);color:#fff;border-radius:6px;padding:6px 10px;font-size:12px;font-weight:600">
            ${servers.map(s => `<option value="${s.id}" data-ws="http://${s.host}:${s.ws_port}${s.path}">${s.name} (${s.host}:${s.port})</option>`).join('')}
          </select>
          <input id="novncPasswordInput" type="password" placeholder="VNC Password (optional)" style="background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.15);color:#fff;border-radius:6px;padding:6px 10px;font-size:12px">
          <button onclick="connectNovncStream()" style="background:#10b981;border:none;color:#000;border-radius:6px;padding:6px 14px;font-size:12px;font-weight:700;cursor:pointer">Connect noVNC Session</button>
        </div>

        <div id="novncDisplayArea" style="flex:1;background:#000;border:1px solid rgba(255,255,255,0.1);border-radius:10px;overflow:hidden;display:flex;align-items:center;justify-content:center;position:relative">
          <div id="novncPlaceholder" style="text-align:center;color:var(--muted);font-size:13px;padding:40px">
            <div style="font-size:36px;margin-bottom:10px">🖥️</div>
            <div style="font-weight:600;color:#fff;margin-bottom:4px">Server Kantor noVNC Ready</div>
            <div>Pilih target server di atas lalu klik <strong>Connect noVNC Session</strong> untuk membuka GUI desktop langsung di browser.</div>
          </div>
          <iframe id="novncIframe" style="width:100%;height:100%;border:none;display:none"></iframe>
        </div>
      </div>
    `;
    ...
    container.innerHTML = `<div style="color:#ef4444;font-size:12px;padding:12px">Failed to load noVNC gateway: ${err.message}</div>`;
  }
}

function connectNovncStream() {
  const sel = document.getElementById('novncSelectServer');
  const iframe = document.getElementById('novncIframe');
  const placeholder = document.getElementById('novncPlaceholder');
  if (!sel || !iframe) return;

  const opt = sel.options[sel.selectedIndex];
  const wsUrl = opt.getAttribute('data-ws') || `http://100.106.207.44:6080/vnc.html`;
...
function promptAddNovncServer() {
  const name = prompt("Nama Server (misal: Server Kantor Linux/Windows):");
  if (!name || !name.trim()) return;
  const host = prompt("IP / Hostname (misal: 100.106.207.44):") || "100.106.207.44";
  const wsPort = prompt("noVNC Websockify Port (default 6080):") || "6080";

  fetch('/api/novnc/save', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      name: name.trim(),
      ...
      target_type: 'office_server'
    })
  }).then(r => r.json()).then(res => {
    if (res.status === 'success') {
      loadNovncPanel();
    } else {
      alert("Failed to save VNC server: " + res.message);
    }
  });
}
